...
HOT 或 OSM 任务处理管理器通常被称为 **TM2，** 作为Tasking Manager版本 2 的简写。 
...
1.点击右上角的 “绘制” 按钮。
2.要绘制表示地图框中感兴趣多边形区域，请按住鼠标右键以移动地图而不单击，这将添加一个节点。多边形应严格限制在感兴趣的区域。这可以节省完成不感兴趣切片（例如海洋、森林）的时间  
3.点击你的起点以完成多边形  
...
### 使用方形网格作为任务方块
要创建统一的方形切片，请选择方形网格选项，然后单击下一步。  
感兴趣区域会自动拆分为网格单元格，每个单元格都成为一项任务。项目面积越大，由此产生的方块就越大。根据五个相对切片大小选项确定最佳切片大小。因此，较小的切片大小将导致更多任务，如屏幕截图所示。因此，最佳切片大小将取决于区域的大小（为大型项目创建更小的切片）和可能需要在每个切片中测绘的要素数量。一般来说，随着感兴趣区域的大小和/或可能的要素数量的增加，为项目选择的切片大小应该减小。 
