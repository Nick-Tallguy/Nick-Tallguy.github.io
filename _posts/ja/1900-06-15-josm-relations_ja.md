...
[ビギナーズガイド](/ja/beginner) では、OpenStreetMapで描画できるオブジェクトには、ポイント（ノード）、ライン（ウェイ）、ポリゴン（閉じたウェイ）の3種類があると説明しました。ラインは複数のポイントを含み、ライン自体が何を表すかを定義する属性を持っています。 ポリゴンは、形状を形成するために始点でラインが終わらなければならないことを除き、ラインと同じです。  
