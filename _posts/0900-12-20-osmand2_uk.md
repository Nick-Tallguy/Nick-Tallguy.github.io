...
мова: англійська
категорія: мобільна картографія
---

OsmAnd 2.x
==========

> Цей путівник може бути завантажений як [osmand2_en.odt](/files/osmand2_en.odt) або [osmand2_en.pdf](/files/osmand2_en.pdf)  
> Переглянуто 2016-08-08  

OsmAnd (OSM Автоматична система навігації) - це додаток із відкритим вихідним  кодом для перегляду, редагування, запису та трансляції мапи, яка використовує базу даних OpenStreeyMap для онлайн та офлайн користування. Він також включає функції навігації та маршрутизації з візуальними та голосовими вказівками, що також доступно в онлайн та офлайн режимах. OsmAnd дозволяє користувачам робити внески безпосередньо до OpenStreetMaps, повідомляючи про помилки, завантажуючи GPX-доріжки, а також додаючи (чи змінюючи) POI.  
...
- Більше інформації про iD на [Налаштування фонового слою](/en/beginner/id-editor/#configuring-the-background-layer).  
- Більше інформації про JOSM на [Відкрити в JOSM](/en/mobile-mapping/using-gps/#open-in-josm)  
...
* Упевніться, що плагін OpenData встановлений в JOSM, після чого відкрийте щойно завантажений файл.   
* Use lines to write the grid numbers in each cell  
* Експортувати як GPX  
* Завантажте на пристрій у теку */osmand/tracks*.  
* Увімкніть в OsmAnd функцію **GPX Tracks..**, а також виберіть GPX-доріжку, із якою працювали та натисніть *OK*. Подивіться попередній розділ цієї документації *Відображення GPX-доріжок*  

GPX-доріжка тепер доступна на мапі OsmAnd.  
...
Згайте трохи часу на практику та ознайомлення із додатком перед початком роботи із справжньою інформацією.   

У цьому розділі представлена концепція використання OsmAnd для збору GPX-доріжок, модифікації або видалення POI, включаючи додавання, коментування чи закриття приміток/помилок, а також інформація про передачу цих файлів на персональний комп'ютер.   
