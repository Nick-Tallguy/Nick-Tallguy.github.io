...
OsmAnd (OSM Automated Navigation Directions) è un'applicazione open source per vedere le mappe, modificarle, registrare tracce e andare in giro che utilizza il database openStreetMap (OSM) per utilizzo sia online che offline. Comprende anche una caratteristica di instradamento e navigazione, con guida visuale e vocale, che è disponibile sia online che offline. OsmAnd consente agli utenti di contribuire direttamente a OpenStreetMap riportando errori, caricando tracce GPX, e aggiungendo (o modificando) Punti di Interesse (POI).  
...
Oltre alle fonti precedenti, una versione mantenuta dalla comunità è disponibile da [F-Droid](https://f-droid.org/app/net.osmand.plus), o attraverso software di terze parti come [Amazon](http://www.amazon.com/OsmAnd-Maps-Navigation/dp/B00D0SA8I8).  
...
+ Premi l'icona **Opzioni** e seleziona **Gestisci i file delle mappe**. Vedi i singoli elementi della *Interfaccia Utente* nella prossima sezione. Se si sta usando la versione gratuita dal Google Play tore, potrebbe comparirà un finestra di dialogo. Se compare, semplicemente selezionate "Ok". La versione f-Droid mantenuta dalla comunità non presenta questa finestra di dialogo.  

Da qui, gestisci tutti i file online raster, i database vettoriali offline, ed i file vocali usati dalla app OmAnd.  

+ Sotto l'la linguetta *Tutti i download* sono visualizzati tutti i file disponibili sul server OsmAnd.  
+ Dalla linguetta *locali* sono elencate le mappe, messaggi vocali e registrazioni audio/video nel vostro dispositivo.  
+ Dalla linguetta "Aggiornamenti" sono mostrati i file locali che sono obsoleti e devono essere aggiornati dal server OsmAnd.  
...
Una delle caratteristiche uniche della versione F-Droid della comunità, è la frequenza illimitata di download che potete fare dalla app stessa. La versione gratuita del Play store è limitata a solo dieci download. Una soluzione a questo limite è di scaricare è di scaricare sul vostro pc il database da http://download.osmand.net/rawindexes/](http://download.osmand.net/rawindexes/), e poi trasferirli manualmente sul vostro dispositivo.  
...
Il recente Android alla versione 2.x (e OsmAnd DVR su iOS fino alla 3.x) migliora l'interfaccia con lo stile Material Design, e ora include un cruscotto con diverse schede, uno stile più amichevole, e modifiche semplificate per i Punti-di-Interesse (POI) di OpenStreetMap e per le Note.  

La mappa mostrerà vie, strade, uso del terreno, POI, corsi d'acqua, e molti altri dati geografici.  

In più, usando i pulsanti dell'interfaccia:  

- Il pulsante "Configurazione Mappa" presenta un menu per selezionare il profilo da usare. Potete scegliere tra: Naviga Mappa. Auto, Bicicletta, Pedoni per specificare quanti più o meno dettagli saranno mostrati. E' anche mostrato 
come i POI, le Etichette, le tracce GPX, le Note OSM e i Favoriti sono visualizzati.  
...
Nel pannello in basso a destra della schermata potete notare **Informazione GPS**, che visualizza il numero di satelliti a cui il vostro dispositivo è connesso, ed il numero di satelliti visibili sull'orizzonte. **Registra Traccia** è visibile quando il plugin "Registrazione Tracce" è abilitato. Lo stesso vale per lo "Strumento Righello", se il plugin "Calcolatore Distanze e Strumento Pianificazione" è abilitato. Ulteriori pulsanti possono o meno essere visibili a seconda di come sono stati impostati o certi plugin sono stati abilitati.  

Quando sul vostro dispositivo è abilitato il GPS o il posizionamento tramite rete, un **Indicatore Posizione** (le vostre coordinate in qualche punto del cerchio blu) visualizza la vostra posizione approssimativa sulla mappa. Premendo il pulsante "Dove Sono" centra la mappa sulla locazione GPS corrente o l'ultima conosciuta. Il contorno dell'icona è blu quando l'informazione posizionale è disponibile, e grigio quando la posizione non è ancora nota.  

Il **Cruscotto** mostra diverse schede informative, le opzioni *Cerca*, *Storia*, *Le mie Tracce*, e *Plugin*.    
...
Il pulsante **Navigazione** apre una finestra di dialogo per fissare le rotte di navigazione Da e A che sono appropriate per la modalità di trasporto scelta (per es. auto, bici, o pedone)  
