...
layout: doc
title: OsmAnd 2.0+
permalink: /uk/mobile-mapping/osmand2/
lang: uk
...
OsmAnd (OSM Automated Navigation Directions) – це застосунок із відкритими сирцями для перегляду, редагування, запису треків та навігації, який використовує дані OpenStreetMap для онлайн та офлайн режимів. Він також включає функції навігації та маршрутизації з візуальними та голосовими підказками, які також доступні в онлайн та офлайн режимах. OsmAnd дозволяє користувачам робити зміни безпосередньо в OpenStreetMaps, повідомляючи про помилки, завантажуючи GPX-треки, а також додаючи (чи змінюючи) POI.  

<a href="https://play.google.com/store/apps/details?id=net.osmand">
  <img alt="OsmAnd для Android"
       src="//lh5.ggpht.com/-hvpyv4R8VQ8-N6ym0FuVFkfW3ZwU6UmhPn-IfxTn2p97bjLt3aDEH53rzPM6XKEszM=w45-rw" />
</a>

Версія для iOS доступна на iTunes.  

<a href="https://itunes.apple.com/app/id934850257">
  <img alt="OsmAnd Maps на iTunes"
       src="http://linkmaker.itunes.apple.com/images/badges/en-us/badge_appstore-lrg.png" />
</a>

>Деякі зфункцій OsmAnd, що є на платформі Android, можуть поки бути недоступними на платформі iOS. Дивіться подробиці у блозі розробників: [blog site](http://osmand.net/blog).  

На додаток до вищезазначених джерел, підтримувана спільнотою версія доступна через [F-Droid](https://f-droid.org/app/net.osmand.plus) або через сторонні джерела програмного забезпечення, такі як [Amazon](http://www.amazon.com/OsmAnd-Maps-Navigation/dp/B00D0SA8I8).  

Початок
---------------

OsmAnd дозволяє використовувати як онлайн-мапи, так і офлайн-бази даних побудовані на основі OpenStreetMap, які не потребують підключення до Інтернету. Останнє передбачає, що необхідні кроки по завантаженню та збереженню бази даних у вашому пристрої виконані.  

Можливість працювати в режимі оффлайн – головна риса OsmAnd. Тому одним із найперших кроків, який новий користувач має зробити – це завантаження даних. Якщо вам потрібні голосові підказки під час руху по маршруту, вам також треба отримати відповідні файли.  

+ Натисніть значок **Options** та оберіть **Manage map files**. Дивіться елементи полотна  *User Interface* у наступному розділі для довідки. Якщо Ви користуєтесь безкоштовною версією з  Google Play Store, може з'явитися діалогове вікно. Якщо це так, просто натисніть "Ok". Версія підтримувана спільнотою з F-Droid не міститиме такого діалогу.  
...
- Більше інформації про [Налаштування фонового шару](/uk/beginner/id-editor/#налаштування-тла) в iD.  
- Більше інформації про JOSM у "[Відкрити в JOSM](/uk/mobile-mapping/using-gps/#open-in-josm)"  
- Для інших редакторів OpenStreetMap зверніться до їхньої документації.   
...
* Переконайтесь, що втулок OpenData встановлений в JOSM, після чого відкрийте щойно завантажений файл.   
* Use lines to write the grid numbers in each cell  
* Експортувати як GPX  
* Завантажте на пристрій у теку */osmand/tracks*.  
* Увімкніть в OsmAnd функцію **GPX Tracks.**, а також виберіть GPX-трек, який ви щойно зробили та натисніть *OK*. Подивіться попередній розділ цієї документації *Показ GPX-треків*  

GPX-трек тепер доступний на мапі OsmAnd.  
...
Витратьте трохи часу на практику та ознайомлення із додатком перед початком роботи зі справжньою інформацією.   

У цьому розділі представлена концепція використання OsmAnd щодо запису GPX-треків, модифікації або вилучення POI, включаючи додавання, коментування чи закриття приміток/помилок, а також інформація про передачу цих файлів на персональний комп'ютер.   

Офіційна документація OsmAnd
-------------------
