...
**Si l'interface utilisateur de la version que vous utilisez semble différente de celle décrite ici, vous devez consulter [notre aperçu de la version](/fr/coordination/tm-disambiguation)**
...
Le gestionnaire de tâches 3 est un outil totalement distinct du projet principal OpenStreetMap. Lorsque vous contribuez à OpenStreetMap à l'aide du gestionnaire de tâches 3, vous utilisez en fait plusieurs outils logiciels :
...
Le rôle du gestionnaire de tâches 3 est de présenter une région géographique spécifique divisée en petites zones rapides à cartographier appelées "Tâches" et de fournir un moyen de "vérifier" ou de "verrouiller" l'une de ces petites zones pendant que vous la cartographiez à l'aide d'un des éditeurs OpenStreetMap standard. En "contrôlant" ou en "verrouillant" une de ces petites zones, vous vous assurez que personne d'autre ne cartographiera cette même zone pendant que vous le faites. Cela permet d'éviter la "double cartographie" ou d'autres erreurs lorsque deux personnes travaillent exactement sur la même zone dans OpenStreetMap.
...
1. Si vous n'avez pas de compte OpenStreetMap, allez sur https://openstreetmap.org et créez-en un. Il s'agira des informations de connexion que vous utiliserez pour vous connecter au gestionnaire de tâches. <br/>
  ![TM Quick Start 1][]
2. Visitez [http://tasks.hotosm.org/](http://tasks.hotosm.org/) et connectez-vous via le coin supérieur droit  
  ![TM Quick Start 2][]
  > Vous devez fournir une adresse électronique valide dans votre profil (accessible par la première entrée du menu dans le coin supérieur droit) avant de pouvoir commencer à cartographier.
...
1. Un cartographe distant crée un compte dans OpenStreetMap et se connecte au gestionnaire de tâches 3.
...
### Gestionnaire de tâches 3 - Connexion
...
Une fois que vous serez connecté à OpenStreetMap, vous serez renvoyé au gestionnaire de tâches. Il est maintenant temps d'ajouter votre adresse électronique au profil. Cliquez sur votre nom d'utilisateur dans le coin supérieur droit pour ouvrir le menu et sélectionnez la première entrée, "Votre profil". Vous pouvez modifier vos coordonnées en utilisant le bouton correspondant dans la partie supérieure droite du formulaire. Le gestionnaire de tâches permet d'envoyer des messages et il est donc nécessaire de fournir une adresse électronique valide avant de pouvoir commencer à établir des cartes.

En cliquant sur Contribuer ou Commencer, vous accéderez à une liste de projets auxquels vous pouvez contribuer.
...
### Trouver un projet - Gestionnaire de tâches 3 Écran de contribution
...
### Cartographie via le gestionnaire de tâches 3
...
- Les carrés de grille roses ont été invalidés par un validateur et sont à nouveau disponibles pour la cartographie.
- Les zones rouges de la carte indiquent des zones plus prioritaires
- Les zones jaunes/or ont subi une première passe de cartographie, et sont prêtes à être validées
- Les tâches bleues sont en cours de réalisation
- Une bordure bleu foncé indique la tâche que vous avez sélectionnée.
...
Vous pouvez revenir à la page principale de contribution au projet en cliquant sur le titre du projet sur la page Activité et statistiques.
...
>  1. L'option 'Activer le contrôle à distance' doit être cochée - sous **Édition / Préférences / Contrôle à distance**.
...
Pour les questions dont la réponse nécessite un peu de recherche ou des explications plus détaillées, le meilleur moyen est d'écrire à l'adresse suivante mapper-support@hotosm.org
...
2. Rejoignez le canal de soutien du cartographe.
